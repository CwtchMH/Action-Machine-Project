...
export default function UnimportantUnurgent({ tasks }: TasksState) {
  ...
  return (
    <div className={`lg:rounded-tl-3xl bg-blue-50 rounded-3xl lg:rounded-none flex flex-col ${toggleInfor ? "" : 'justify-center'}`}>
      ...
      {/* {tasks?.unimportant?.unurgent &&
        tasks?.unimportant?.unurgent.map((task: string, index: number) => (
          <p
            key={index}
            className="mt-2 bg-blue-100 cursor-pointer rounded-lg text-sm text-black capitalize flex px-3 py-2 mx-2 justify-center text-base shadow-sm border border-gray-300 transition-transform duration-300 ease-in-out hover:scale-105 hover:shadow-md break-all"
          >
            {index + 1}. {task}
          </p>
        ))} */}
