...
export default function ImportantUrgent({ tasks }: TasksState) {
  ...
  const [taskList, setTaskList] = React.useState<Task[]>(taskArray)
  ...
  const dragEndHandler = (event: DragEndEvent) => {
    ...
    setTaskList((taskList) => {
      ...
      // Sắp xếp lại vị trí bằng arrayMove
  ...
  const dragOverHandler = (e: DragOverEvent) => {
    ...
    setTaskList((taskList) => {
      if (initialContainer === targetContainer) {
        const orginalIndex = taskList.findIndex((task) => task.id === active.id)
        const targetIndex = taskList.findIndex((task) => task.id === over.id)
        return arrayMove(taskList, orginalIndex, targetIndex)
      }
      return taskList
    })
  }
  ...
  return (
    ...
    >
      ...
      {/* Information part */}
      <DndContext onDragEnd={dragEndHandler} onDragOver={dragOverHandler} sensors={sensors}>
        <TasksDisplay
      ...
      >
        ...
        <div
          className={`w-fit mx-auto ${taskArray.length === 0 ? 'hidden' : ''}`}
